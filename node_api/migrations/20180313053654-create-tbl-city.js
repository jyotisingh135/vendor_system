  ...
  up: (queryInterface, Sequelize) => {
    return queryInterface.createTable('tblCities', {
      ...
      updatedAt: {
        ...
        let sql="";
        for(let i=0;i<cityJSON.length/2;i++)
            sql+="("+cityJSON[i].id+", '"+cityJSON[i].name+"', '"+cityJSON[i].state+"'),";

        return queryInterface.sequelize.query("Insert into tblCities(cityId, cityName, state) values"+sql.slice(0, -1)+";")
    });
  },
